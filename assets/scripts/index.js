...
const hexCopy = document.querySelector(`input[name='hexCopy']`);
...
function colorValidate(isValidFunc, value) {
   if (isValidFunc(value)) {
      hexCopy.disabled = false;
      rgbCopy.disabled = false;
      hslCopy.disabled = false;
   } else {
      hexCopy.disabled = true;
      rgbCopy.disabled = true;
      hslCopy.disabled = true;
   }
}

hexCopy.addEventListener('click', copyHandler(hexInput));
rgbCopy.addEventListener('click', copyHandler(rgbInput));
hslCopy.addEventListener('click', copyHandler(hslInput));
genButton.addEventListener('click', generateHandler);

function copyHandler(input) {
   return function () {
      let color = StrBeautify(input.value);
